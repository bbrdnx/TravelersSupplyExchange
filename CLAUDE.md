...
| Date | Change |
|------|--------|
| 2026-06-24 | Immersive mobile view. On phone viewports (`@media max-width:600px`) the app now fills the whole screen like a real app: `body` padding/gap/gray bg removed, `.phone` goes `100vw`/`100dvh` with no border-radius, box-shadow, or notch (uses `dvh` so it sits correctly under browser chrome). The side caption no longer stacks below requiring a scroll: on mobile it's hidden behind a floating "ⓘ Guide" button (`.guide-fab`, fixed bottom-right above the tab bar) that opens the caption as a bottom-sheet overlay (`.caption.open` + `.guide-backdrop`), with an in-sheet `×` close (`.guide-close`). New JS `openGuide()`/`closeGuide()`; `go(id)` now calls `closeGuide()` so tapping a tab or step-link dismisses the overlay and reveals the screen. Desktop layout unchanged (fab/backdrop/close hidden above 600px). Verified with jsdom (9 checks pass). |
